...
All features default to UNAPPROVED.

Approved features are only those explicitly marked Approved = O.

Workflow progression does not imply approval.
...
- Total features
- Approved features (explicitly Approved = O only)
...
Approved count must never imply bulk approval.
Default feature state remains UNAPPROVED.
...
5. Workflow progression never overrides governance gates.
...
- Any Vanilla Test category < 4 → DO NOT IMPLEMENT.
...
# V. VANILLA TEST ENFORCEMENT

Each category must score ≥ 4.
...
Any category below 4 → DO NOT IMPLEMENT.
